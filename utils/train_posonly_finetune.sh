...
then
  ...
  base_lr=$4
  snapshot_prefix=$snapshot_prefix"_lr"$base_lr
fi
...
preproc="{'data_mean':'/om/user/hyo/caffe/imagenet_mean.npy','crop_size':227,'do_img_flip':True,'noise_level':10}"
  ...
  python /om/user/hyo/metrics/neural_spatial/utils/train.py -n $net -l $base_lr -c $lr_policy -g $gamma -z $stepsize -e $max_iter -m $momentum -i $iter_size -w $weight_decay -s $snapshot -p $snapshot_prefix -f $weights -t $test_interval --dp-params=$dp_params --preproc=$preproc --do-test=0
